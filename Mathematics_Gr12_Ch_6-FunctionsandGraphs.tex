\chapter{Functions and Graphs - Grade 12}
\label{m:fg12}
...
% FIXME: axes labels intersect the circle
...
The inverse function of $y=ax^2$ is determined by solving for $x$ as follows:
\begin{eqnarray}
y&=&a^x\\
\log(y)&=&\log(a^x)\\
&=&x\log(a)\\
...
\end{eqnarray}
The inverse of $y = 10^x$ is $x = 10^y$, which we write as $y=\log(x)$.  Therefore, if $f(x) = 10^x$, then $f^{-1} = \log(x)$.
...
\begin{enumerate}
\item{Write down the inverse in the form $h^{-1}(x) =...$}
