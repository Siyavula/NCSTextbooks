...
The natural logarithm is to the base $e$ which is approximately $2.71828183...$. $e$ is like $\pi$
...
\Activity{Logarithm Law 3: $\log_{a}(x\cdot y) = \log_{a}(x) + \log_{a}(y)$}{Write as seperate logs:
\begin{enumerate}
\item{$\log_{2}(8\times4)$}
...
\Activity{Logarithm Law 4: $\log_{a}\left(\frac{x}{y}\right) = \log_{a}(x) - \log_{a}(y)$}{Write as seperate logs:
\begin{enumerate}
...
\nequ{(2^3)^{\frac{2}{3}}+\log_2 2^5=(2)^{3\frac{2}{3}}+5\log_2 2}

\westep{Determine which laws can be used.}
We can now use $\log_a a =1$

\westep{Apply log laws to simplify}
\nequ{(2)^{3\frac{2}{3}}+5\log_2 2=(2)^2+5(1)=4+5=9}
...
\nequ{7 \cdot 5^{(-3\frac{2}{3}+3)}=7 \cdot 5^{(-2+3)}=7 \cdot 5^{1}=35 \quad \checkmark}
}\end{wex}
...
\begin{wex}{}{}{}{Logs in Compound Interest}{I have R12~000 to invest.  I need the money to grow to at least R30~000.  If it is invested at a compound interest rate of 13\% per annum, for how long (in full years) does my investment need to grow ? }{
