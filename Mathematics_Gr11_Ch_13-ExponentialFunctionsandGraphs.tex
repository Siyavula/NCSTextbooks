...
\begin{enumerate}
\item{$a(x)=-2\cdot b^{(x+1)} + 1$}
\item{$b(x)=-1\cdot b^{(x+1)} + 1$}
\item{$c(x)=0\cdot b^{(x+1)} + 1$}
\item{$d(x)=1\cdot b^{(x+1)} + 1$}
\item{$e(x)=2\cdot b^{(x+1)} + 1$}
\end{enumerate}
Use your results to deduce the effect of $a$.}
\item{On the same set of axes, plot the following graphs:
\begin{enumerate}
\item{$f(x)=1\cdot b^{(x+1)} -2$}
\item{$g(x)=1\cdot b^{(x+1)} -1$}
\item{$h(x)=1\cdot b^{(x+1)} +0$}
\item{$j(x)=1\cdot b^{(x+1)} +1$}
\item{$k(x)=1\cdot b^{(x+1)} +2$}
\end{enumerate}
Use your results to deduce the effect of $q$.}
\item{Following the general method of the above activities, choose your own values of $a$ and $q$ to plot 5 different graphs of $y=ab^{(x+p)} + q$ to deduce the effect of $p$.}
\end{enumerate}}
...
\psplot[plotstyle=curve,arrows=<->]{-5}{0.5}{2 x 1 add exp 3 mul 2 add}
\end{pspicture}
