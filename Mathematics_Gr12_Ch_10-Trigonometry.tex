...
\begin{wex}{Trigonometric Identities 3}{Prove that $$\frac{\sin \theta+\sin  2\theta}{1+\cos\theta+\cos 2\theta}=\tan \theta$$
...
\Exercise{}{
\begin{enumerate}
\item The line $BC$ represents a tall tower, with $C$ at its foot. Its angle of elevation from $D$ is $\theta$. We are also given that $BA=AD=x$.\\
...
\item Find $BC$ if we are given that $x=140m$, $\alpha=21\degree$ and $\theta=9\degree$.
\end{enumerate}
