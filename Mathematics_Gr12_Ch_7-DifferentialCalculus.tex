...
to travel the 1 000~m head start that the tortoise had. However, in these 500$\es$, the tortoise has travelled a further
...
Given two functions, $f(x)$ and $g(x)$ and a constant $b$, we know that:
\begin{center}
\begin{tabular}{l}
$\frac{d}{dx} b = 0$\\
\\
...
A graph has a horizontal \textbf{point of inflexion} where the derivative is zero but the sign of the gradient does not change. That means the graph always increases or always decreases.
\begin{center}
...
\begin{wex}
{Optimisation Problems}{Michael wants to start a vegetable garden, which he decides to fence off in the shape of a rectangle from the rest of the garden. Michael has only 160~m of fencing, so he decides to use a wall as one border of the vegetable garden. Calculate the width and length of the garden that corresponds to largest possible area that Michael can fence off.
