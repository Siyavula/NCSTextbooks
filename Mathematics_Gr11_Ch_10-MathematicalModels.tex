...
\begin{eqnarray*}
&5 &\times R3 \quad \mbox{for washing the dishes}\\
+&2 &\times R5 \quad \mbox{for cutting the grass}\\
=& R15&+R10\\
=& R25
\end{eqnarray*}
If however, we ask: "What is the equation if Anna helps her mother $x$ times and her father $y$ times?", then we have:

\nequ{\mbox{Total earned}=x \times R3 + y \times R5}
...
\item{Erica has decided to treat her friends to coffee at the Corner Coffee House. Erica paid R54,00 for four cups of cappuccino and three cups of filter coffee. If a cup of cappuccino costs R3,00 more than a cup of filter coffee, calculate how much each type of coffee costs?}
...
\nequ{s = 5t^2 + v_0t}
In this equation, $v_0$ is the initial velocity, in \ms. Distance is measured in meters and time is measured in seconds. Use the equation to find how far an object will fall in 2~s if it is thrown downward at an initial velocity of 10~\ms.}
...
The object will fall 40~m in 2~s if it is thrown downward at an initial velocity of 10~\ms.}
\end{wex}
...
\nequ{s = 5t^2 + v_0t}
In this equation, $v_0$ is the initial velocity, in \ms. Distance is measured in meters and time is measured in seconds. Use the equation find how long it takes for the object to reach the ground if it is dropped from a height of 2000~m. The initial velocity is 0~\ms.}
{
\westep{Identify what is given for each problem}
...
\westep{Write the final answer}
The object will take 20~s to reach the ground if it is dropped from a height of 2000~m.}
\end{wex}
...
\begin{enumerate}
\item How far does the car travel in 20~s?
\item How long does it take the car to travel 300~m?
\end{enumerate}}
...
\begin{wex}{Setting up an equation}{Currently the subscription to a gym for a single member is R1~000 annually while family membership is R1~500.  The gym is considering raising all memberships fees by the same amount.  If this is done then the single membership will cost $\frac{5}{7}$ of the family membership.  Determine the proposed increase.}{
